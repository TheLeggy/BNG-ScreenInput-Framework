...
local function initSecondStage(jbeamData)
  ...
  -- Register screen dimensions with screenService for trigger box sizing
  -- This eliminates the need for a separate screenConfigs.jsonc file
  obj:queueGameEngineLua([[
    if screenService and screenService.configureScreen then
      screenService.configureScreen("]]..screenName:gsub("@", "")..[[", {
        width = ]]..width..[[,
        height = ]]..height..[[
      })
    end
